...
class Lens():
    ...
    def get_field(self,client,config,log=None,debug=False,testing=False):
    ...
    def get_operator(self,client,config,log=None,debug=False,testing=False):
    ...
    def check_event_name_in_DB(self,client,config,log=None,debug=False,testing=False):
        ...
        self.eventname_pk = int(api_tools.contact_db(client,config,params,
    ...
    def check_event_name_assoc_event(self,client,config,log=None,debug=False,testing=False):
        ...
        self.got_eventname = bool(api_tools.contact_db(client,config,params,
    ...
    def check_event_in_DB(self,client,config,log=None,debug=False,testing=False):
    ...
    def add_event_to_DB(self,client,config,log=None,debug=False,testing=False):
        ...
        self.get_field(client,config,log=log,debug=debug,testing=testing)
        self.get_operator(client,config,log=log,debug=debug,testing=testing)
    ...
    def check_last_singlemodel(self,client,config,log=None,debug=False,testing=False):
        ...
        ts = datetime.strptime(response.split(' ')[1],"%Y-%m-%dT%H:%M:%S")
        ts = ts.replace(tzinfo=pytz.UTC)
        self.singlemodel_ts = ts
        
        if log != None:
            log.info(' -> Last singlemodel '+str(self.singlemodel_pk)+\
                    ' updated at '+self.singlemodel_ts.strftime("%Y-%m-%dT%H:%M:%S"))
        
    def add_singlemodel_to_DB(self,client,config,log=None,debug=False,testing=False):
        ...
        self.check_last_singlemodel(client,config,log=log,debug=debug,testing=testing)
            
    def sync_event_with_DB(self,client,config,log=None,debug=False,testing=False):
        ...
        self.check_event_in_DB(client,config,log=log,debug=debug,testing=testing)
        if self.event_pk == -1:
            self.add_event_to_DB(client,config,log=log,debug=debug,testing=testing)
        
        self.check_event_name_in_DB(client,config,log=log,debug=debug,testing=testing)
        if self.eventname_pk == -1:
            self.add_eventname_to_DB(client,config,log=log,debug=debug,testing=testing)
            
        self.check_last_singlemodel(client,config,log=log,debug=debug,testing=testing)
        if self.singlemodel_pk == -1 or self.last_updated > self.singlemodel_ts:
            self.add_singlemodel_to_DB(client,config,log=log,debug=debug,testing=testing)
