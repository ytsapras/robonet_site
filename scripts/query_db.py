        ...
        if qs.count() == 0:
            ...
            for q in qs:
                log.info(' '.join([q.grp_id, q.field.name,\
                            observation_classes.get_request_desc(q.request_type), \
...
def get_event_field_id(ra_str,dec_str):
    ...
    (id_field, rate) = field_check.romecheck(ev_ra_deg, ev_dec_deg)

    if id_field == -1:
        id_field = 'Outside ROMEREA footprint'
    else:
        id_field = sorted(rome_fields_dict.field_dict.keys())[id_field]
